...
def secrets_dir(tmp_path):
    """Return a temporary directory and set MERIDIAN_SECRETS_DIR to point at it."""
    return tmp_path
...
class TestGetSecretsDir:
    def test_raises_when_env_var_not_set(self, monkeypatch):
        ...
        with pytest.raises(RuntimeError, match="MERIDIAN_SECRETS_DIR"):
            get_secrets_dir()

    def test_raises_when_directory_does_not_exist(self, monkeypatch, tmp_path):
        missing = tmp_path / "nonexistent"
        monkeypatch.setenv("MERIDIAN_SECRETS_DIR", str(missing))
...
class TestLoadClients:
    def test_loads_single_valid_config(self, monkeypatch, secrets_dir):
        monkeypatch.setenv("MERIDIAN_SECRETS_DIR", str(secrets_dir))
